...
import { formatTasks, parseTasks, writeTasks } from "../tasks/index.js";
...
export async function handleDailyTasksCommand(userText: string): Promise<string | null> {
  const tokens = normalizeTokens(userText);
  if (!tokens.length) return null;

  const hasTasks = tokens.some(token =>
    token === "task" ||
    token === "tasks" ||
    token === "todo" ||
    token === "todos" ||
  ...
  const wantsEdit = tokens.some(token => token === "edit" || token === "update" || token === "change");
  ...
  if (wantsEdit) {
    const match = userText.match(/\b(?:edit|update|change)\s+(?:daily\s+)?tasks?\s*[:\-]?\s*(.+)$/i);
